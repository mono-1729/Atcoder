...
bd=sorted(list(set(b+d)))
mul=MultiSet(compress=bd)
chocobox=[(a[i],b[i],0) for i in range(n)]
for i in range(m):
	chocobox.append((c[i],d[i],1))
chocobox.sort(reverse=True)
count=0
for i in range(n+m):
	x,y,f=chocobox[i]
	if f==0:
		num=mul.bisect_left(y)
		if num==count:
			print('No')
			exit()
		else:
